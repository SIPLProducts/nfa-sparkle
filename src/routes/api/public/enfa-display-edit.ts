...
export const Route = createFileRoute("/api/public/enfa-display-edit")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { callEnfaDisplayEditData } = await import("@/lib/sap-report.server");
        ...
        if (!result.ok) {
          ...
          console.warn("[enfa-display-edit] SAP call failed:", result.status, result.error);
