...
export const Route = createFileRoute("/api/public/sap-plant")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { callSapPlantF4 } = await import("@/lib/sap-report.server");
        ...
        try {
          ...
          bukrs = String(input?.["bukrs"] ?? "").trim();
        } catch {
          bukrs = "";
        }
        if (!bukrs) {
          return Response.json({ error: "A company code (bukrs) is required" }, { status: 400 });
        }

        let result;
        try {
          result = await callSapPlantF4(bukrs);
        } catch (error) {
          const message = error instanceof Error ? error.message : "Plant service failed";
          return Response.json(
            { error: `Could not load plants from SAP: ${message}` },
