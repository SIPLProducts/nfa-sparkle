...
export const Route = createFileRoute("/api/public/sap-enfa-type")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { callSapEnfaTypeF4 } = await import("@/lib/sap-report.server");
        ...
        let result;
        try {
          result = await callSapEnfaTypeF4();
        } catch (error) {
          const message = error instanceof Error ? error.message : "NFA type service failed";
          return Response.json(
            { error: `Could not load NFA types from SAP: ${message}` },
