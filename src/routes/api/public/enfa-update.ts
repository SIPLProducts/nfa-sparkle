...
export const Route = createFileRoute("/api/public/enfa-update")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { callEnfaUpdate } = await import("@/lib/sap-report.server");
        ...
        const result = await callEnfaUpdate(input);
        ...
        if (!result.ok) {
          console.error("[enfa-update] SAP call failed:", result.status, result.error);
          return new Response(
